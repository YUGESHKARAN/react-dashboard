...
function LiquidGraph({product5}) {
  const xValues = product5.map(item => item.productName);
  const yValues = product5.map(item => item.price);
  const data = {
    labels: xValues,
    datasets: [
      {
  ...
  return (
    ...
    <h1 className='mb-3 font-bold'>Egg Liquid Products</h1>
