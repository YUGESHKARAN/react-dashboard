...
function EggGraph({product3}) {
  const xValues = product3.map(item => item.productName);
  const yValues = product3.map(item => item.price);
  const data = {
    labels: xValues,
    datasets: [
      {
  ...
  const options = {
    ...
    scales: {
      ...
      x: {
        ...
        ticks: {
            font: {
                size: 6, // Change font size of y-axis
  ...
  return (
    ...
    <h1 className='mb-3  font-bold'>White Egg Products</h1>
